...
def rel_path(*parts):
    return join(dirname(abspath(__file__)), *parts)

# imports 
def import_image(*path, alpha = True, format = 'png'):
	full_path = rel_path(*path) + f'.{format}'
...
def import_folder(*path):
	...
	frames = []
	for folder_path, sub_folders, image_names in walk(rel_path(*path)):
...
def import_sub_folders(*path):
	frames = {}
	for _, sub_folders, __ in walk(rel_path(*path)):
		if sub_folders:
...
def all_character_import(*path):
	new_dict = {}
	for _, __, image_names in walk(rel_path(*path)):
